...
def usage_message():
    print '[USAGE]       python script.py   $ORGANIZATION  $REPO  $BRANCH  $CIRCLE_API_TOKEN'
...
def main(argv):
    if len(argv) == 0 or len(argv) < 4 or '-h' in argv or '--help' in argv:
        usage_message()

    artifacts = list_build_artifacts(argv[0], argv[1], argv[2], argv[3])
